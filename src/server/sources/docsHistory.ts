...
export const loadDocsHistory = unstable_cache(async () => {
  const results = await Promise.allSettled(['docs', 'platform-includes'].map(async path => {
  ...
  if (!groups.length) throw results.find(result => result.status === 'rejected')?.reason;
